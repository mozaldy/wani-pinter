...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ModalProvider students={students} insights={insights}>
      <div className="app-shell" style={{ display: 'grid', gridTemplateColumns: '260px 1fr', minHeight: '100vh', background: 'var(--color-bg)' }}>
        <Sidebar user={user} />
        <main>
          <Topbar />
          <div className="app-content" style={{ padding: '24px 32px 64px', maxWidth: 1440, margin: '0 auto', width: '100%' }}>
            {children}
          </div>
        </main>
      </div>
